...
for (let i = 0; i < testCount; i++) {
  ...
  test(`test for address ${userAddress} with name variation ${char}`, async ({ page }) => {
...
if (addresses.length > 0) {
  ...
  test(`test for address ${userAddress} with custom tester name '"TESTER'`, async ({ page }) => {
    await page.setViewportSize({ width: 1600, height: 1200 });
    // eportal run
    await page.goto(eportalUrl);
    await page.getByRole('banner').getByRole('link', { name: 'Moving Services' }).click();
...
// run test debug: npx playwright test SPC.spec.js --debug